let searchContent = getUrlParams().get('search');
console.log(searchContent);

const fetchProducts = (page = 1, sort = "", search = "") => {
  callApi(`products?name_like=${search}&_page=${page}&_limit=5${sort}`)
    .then((res) => {
...
const renderProduct = (productList, page) => {
  ...
  for (let i = 1; i <= numOfPage; i++) {
    content += `<li class="page-item ${
      page == i ? "active" : ""
    }"><a class="page-link" href="#" onclick="fetchProducts('${i}')">${i}</a></li>`;
  }
  content += `<li class="page-item">
...
getElById("filterProduct").addEventListener("change", () => {
  ...
  fetchProducts(`1`, `&_sort=price&_order=${filterPro}`,searchContent );
});
